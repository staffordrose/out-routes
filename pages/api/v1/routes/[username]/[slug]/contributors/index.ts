...
import { PaginatedContributors } from '@/types';
import {
  allowApiMethods,
  catchApiResponse,
  getQueryParam,
...
type GetResponse = NextApiResponse<PaginatedContributors> | void;
  ...
  try {
    ...
    if (typeof route.is_private !== 'boolean' || route.is_private) {
      ...
      const authUserId = await getUserIdOrThrow(
        session,
        `You must be signed in to view the route contributors`
      );
    ...
    const contributors = await getRouteContributorsByUsernameSlug(
      username,
      slug,
      { size, after }
    );
